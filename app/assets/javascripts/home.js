    ...
    var KB = ( function(KB) {
		...
		$(function(){
			...
			$(window).resize(function() {
				...
				if( domWidth <= 960 ) {
					//reduce the font-size from 10px down
					$('body').css('font-size', domWidth/96 + 'px')
				}
			});
			...
			$('.gallery .tab').click(function(){
				...
				if($gallery.hasClass('show')){
					...
					$gallery.animate({right:'100%'})
				} else {
					$gallery.addClass('show');
					$gallery.animate({right:'40%'})
				}
			});

			// swap the main photo when you click on a thumbnail
			$('.gallery .thumbnail a').click(function(e){
				e.preventDefault();
				var source = $(this).attr('href');
				source = source.replace("/small/","/large/");
				$('.gallery .photo img').attr('src',source);
			});
		});
